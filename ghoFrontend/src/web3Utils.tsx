...
const GHOContract = new ethers.Contract(GHOAddress, tokenABI, wallet);
...
async function getBalanceOfGHO()
{
    const GHOBalance = await GHOContract.balanceOf(wallet);
    const ghoDecimals = getDecimals();
    return ethers.formatUnits( GHOBalance, await ghoDecimals);
}

async function transferGHO(receiverAddress: string, amount: any)
{
    //const amountToTransferFormated = ethers.parseEther(amount, decimals);
    const tx = await GHOContract.transfer(receiverAddress, amount)
    return tx;
}
...
async function main()
{
    ...
    const nativeBalance = getNativeBalance(wallet.address);
    console.log(await nativeBalance);

    const ghoDecimals = getDecimals();
    console.log(await ghoDecimals);
    
    const ghoBalance = getBalanceOfGHO();
    console.log(await ghoBalance);

    const amountToTransfer = '1';
    const amountToTransferFormated = ethers.parseEther(amountToTransfer,ghoDecimals);
    const resultTx = transferGHO(receiver, amountToTransferFormated);
}
